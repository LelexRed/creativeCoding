...
function start() {
  ...
  // création grille double for / nested loop : répéter dans les 2 sens 
  for (let i = 0; i < number; i++) {
    for (let j = 0; j < number; j++) {
      tiles.push(
        // intégration des paramètres de tiles dans la boucle
        new Tile(
          i * size + size / 2,
          j * size + size / 2,
          size,
  ...
  document.addEventListener("click", function (event) {
    // on vérifie si l'utilisateur a cliqué sur un tile / lient = ajout d'événements 
    tiles.forEach((tile) => {
      if (
